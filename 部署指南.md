# RAG Knowledge System - 部署指南

## 项目简介

基于 FastAPI + LlamaIndex 的检索增强生成（RAG）系统，支持文档向量化检索和智能问答。

### 核心功能
- 📄 文档上传和向量化存储
- 🔍 语义检索和相似度匹配
- 🤖 基于 OpenAI 的智能问答
- 💾 元数据和向量数据分离存储

### 技术栈
- **后端框架**: FastAPI 0.109+
- **AI 框架**: LlamaIndex 0.10+
- **LLM/Embedding**: OpenAI (GPT + text-embedding-ada-002)
- **向量数据库**: Milvus 2.3.13
- **关系数据库**: MySQL 8.0
- **缓存层**: Redis 7.2
- **全文搜索**: Elasticsearch 8.11

## 架构组件

### 中间件服务

| 服务 | 版本 | 用途 | 状态 |
|------|------|------|------|
| **MySQL** | 8.0 | 存储文件元数据和结构化数据 | ✅ 已集成 |
| **Milvus** | 2.3.13 | 向量数据库，存储文档 embedding | ✅ 已集成 |
| **Redis** | 7.2-alpine | 缓存和会话管理 | 🔄 为未来功能预留 |
| **Elasticsearch** | 8.11.0 | 全文搜索引擎 | 🔄 为未来功能预留 |
| **MinIO** | 2023-03-20 | Milvus 依赖的对象存储 | ✅ 已部署 |
| **Etcd** | 3.5.5 | Milvus 依赖的元数据管理 | ✅ 已部署 |

### 系统架构

```
┌─────────────────────────────────────────────────────────────┐
│                        FastAPI 应用                          │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐     │
│  │   文件上传    │  │   向量检索    │  │   智能问答    │     │
│  └──────────────┘  └──────────────┘  └──────────────┘     │
└───────────┬─────────────────┬───────────────────┬─────────┘
            │                 │                   │
    ┌───────▼────────┐ ┌─────▼──────┐    ┌──────▼──────┐
    │  MySQL 8.0     │ │  Milvus    │    │  OpenAI API │
    │  (元数据存储)   │ │ (向量存储)  │    │ (LLM/Embed) │
    └────────────────┘ └────┬───────┘    └─────────────┘
                            │
                    ┌───────┴────────┐
                    │  Etcd + MinIO  │
                    └────────────────┘

未来扩展（已部署服务）:
    ┌────────────────┐    ┌─────────────────┐
    │  Redis         │    │ Elasticsearch   │
    │  (缓存/会话)    │    │ (全文搜索)       │
    └────────────────┘    └─────────────────┘
```

---

## 快速开始

### 1. 环境准备

**服务器要求：**
- Docker >= 20.10
- Docker Compose >= 2.0
- 可用内存 >= 4GB（推荐 8GB）
- 可用磁盘 >= 20GB

**安装 Docker（如未安装）：**

```bash
# Ubuntu/Debian
curl -fsSL https://get.docker.com | sh
sudo usermod -aG docker $USER

# 安装 docker-compose
sudo apt install docker-compose-plugin

# 验证安装
docker --version
docker-compose --version
```

**CentOS/RHEL：**

```bash
# 安装 Docker
sudo yum install -y yum-utils
sudo yum-config-manager --add-repo https://download.docker.com/linux/centos/docker-ce.repo
sudo yum install docker-ce docker-ce-cli containerd.io docker-compose-plugin

# 启动 Docker
sudo systemctl start docker
sudo systemctl enable docker
```

### 2. 配置环境变量

**克隆项目后，配置环境变量：**

```bash
cd /path/to/rag_project

# .env 文件已存在，直接修改敏感信息
vim .env
```

**必须修改的配置：**

```env
# 修改数据库密码（生产环境必须使用强密码）
DB_PASSWORD=your_secure_password_here
DB_ROOT_PASSWORD=your_root_password_here

# 修改 Redis 密码
REDIS_PASSWORD=your_redis_password_here

# 配置 OpenAI API Key（必需）
OPENAI_API_KEY=sk-your-actual-api-key
```

### 3. 启动所有服务

**启动所有中间件（不启动后端应用）：**

```bash
# 仅启动中间件服务
docker-compose up -d mysql_db milvus-standalone redis elasticsearch etcd minio

# 查看服务状态
docker-compose ps

# 查看服务日志
docker-compose logs -f
```

**预期输出：**

```
NAME                    STATUS              PORTS
rag_mysql              Up 30 seconds       0.0.0.0:3306->3306/tcp
milvus-standalone      Up 30 seconds       0.0.0.0:19530->19530/tcp, 0.0.0.0:9091->9091/tcp
rag_redis              Up (healthy)        0.0.0.0:6379->6379/tcp
rag_elasticsearch      Up (healthy)        0.0.0.0:9200->9200/tcp, 0.0.0.0:9300->9300/tcp
milvus-etcd            Up 30 seconds       2379/tcp
milvus-minio           Up (healthy)        9000/tcp
```

**启动包括后端应用在内的所有服务：**

```bash
# 启动所有服务（包括 FastAPI 后端）
docker-compose up -d

# 查看后端日志
docker-compose logs -f backend
```

### 4. 验证服务可用性

**测试 MySQL：**

```bash
docker exec rag_mysql mysql -urag_user -p"$(grep DB_PASSWORD .env | cut -d'=' -f2)" -e "SELECT 1"
# 预期输出: 1
```

**测试 Redis：**

```bash
docker exec rag_redis redis-cli -a "$(grep REDIS_PASSWORD .env | cut -d'=' -f2)" ping
# 预期输出: PONG
```

**测试 Elasticsearch：**

```bash
curl -X GET "http://localhost:9200/_cluster/health?pretty"
# 预期输出: JSON 格式的集群健康信息，status 为 "green" 或 "yellow"
```

**测试 Milvus：**

```bash
curl http://localhost:19530/healthz
# 预期输出: 健康状态信息
```

---

## 本地电脑访问中间件

> **⚠️ 重要说明：**
> - **服务器（当前主机）**：10.112.15.13（运行 Docker 服务的机器）
> - **其他电脑**：需要连接到服务器使用中间件的电脑
> - **"本地电脑"** = "其他电脑"（不是服务器本身）

### 访问方式概览

| 服务 | 从其他电脑访问的地址 | 认证信息 | 默认端口 |
|------|---------------------|----------|---------|
| **MySQL** | `10.112.15.13:3306` | 用户名：`rag_user`<br/>密码：见 `.env` | 3306 |
| **Redis** | `10.112.15.13:6379` | 密码：见 `.env` | 6379 |
| **Elasticsearch** | `10.112.15.13:9200` | 无认证（开发模式） | 9200 |
| **Milvus API** | `10.112.15.13:19530` | 无认证 | 19530 |
| **Milvus Web UI** | `http://10.112.15.13:9091` | 浏览器访问 | 9091 |

### 方法 1：直接连接（内网 IP）

**适用场景：** 其他电脑和服务器在同一内网

**在其他电脑上的 Python 连接示例：**

```python
# ============================================
# 以下代码在其他电脑上运行（不是在服务器上！）
# ============================================

# MySQL 连接
import pymysql

conn = pymysql.connect(
    host='10.112.15.13',  # 服务器IP
    port=3306,
    user='rag_user',
    password='rag_password',  # 从服务器的 .env 文件获取
    database='rag_data',
    charset='utf8mb4'
)

# Redis 连接
import redis

r = redis.Redis(
    host='10.112.15.13',  # 服务器IP
    port=6379,
    password='redis_secure_password',  # 从服务器的 .env 文件获取
    decode_responses=True
)

# Elasticsearch 连接
from elasticsearch import Elasticsearch

es = Elasticsearch(['http://10.112.15.13:9200'])  # 服务器IP
print(es.info())

# Milvus 连接
from pymilvus import connections

connections.connect(
    alias="default",
    host='10.112.15.13',  # 服务器IP
    port=19530
)
```

**在其他电脑上的命令行测试：**

```bash
# ============================================
# 以下命令在其他电脑的终端执行（不是在服务器上！）
# 注意：使用服务器IP 10.112.15.13，不是 localhost
# ============================================

# 测试 Redis（在其他电脑上执行）
redis-cli -h 10.112.15.13 -p 6379 -a redis_secure_password ping
# 预期输出: PONG

# 测试 MySQL（在其他电脑上执行）
mysql -h 10.112.15.13 -P 3306 -urag_user -p
# 输入密码 rag_password

# 测试 Elasticsearch（在其他电脑上执行）
curl http://10.112.15.13:9200/_cluster/health?pretty
# 预期输出: JSON格式的集群健康信息

# 测试 Milvus（在其他电脑上执行）
curl http://10.112.15.13:19530/healthz
# 预期输出: 健康状态信息

# 在浏览器打开 Milvus Web UI（在其他电脑上）
# 访问：http://10.112.15.13:9091
```

### 方法 2：SSH 隧道（推荐，安全）⭐

**适用场景：**
- 需要加密传输保护数据安全
- 生产环境的最佳实践
- 不想直接暴露数据库端口

**工作原理：**
```
其他电脑(localhost:6379) <--SSH隧道--> 服务器(10.112.15.13:6379)
           ↑                                      ↑
    在其他电脑上访问                         服务器上的Redis
    使用 localhost
```

#### 2.1 在其他电脑上建立 SSH 隧道

**单个端口映射示例：**

```bash
# ============================================
# 以下命令在其他电脑的终端执行（不是在服务器上！）
# ============================================

# 映射 MySQL（其他电脑的localhost:3306 → 服务器的localhost:3306）
ssh -L 3306:localhost:3306 yl@10.112.15.13 -N -f

# 映射 Redis
ssh -L 6379:localhost:6379 yl@10.112.15.13 -N -f

# 映射 Elasticsearch
ssh -L 9200:localhost:9200 yl@10.112.15.13 -N -f

# 映射 Milvus API
ssh -L 19530:localhost:19530 yl@10.112.15.13 -N -f

# 映射 Milvus Web UI
ssh -L 9091:localhost:9091 yl@10.112.15.13 -N -f
```

**一键映射所有端口（推荐）：**

```bash
# ============================================
# 在其他电脑的终端执行这一条命令即可
# ============================================

ssh -L 3306:localhost:3306 \
    -L 6379:localhost:6379 \
    -L 9200:localhost:9200 \
    -L 9300:localhost:9300 \
    -L 19530:localhost:19530 \
    -L 9091:localhost:9091 \
    yl@10.112.15.13 -N -f

# 输入服务器SSH密码后，隧道会在后台运行
# 注意：yl 是服务器的SSH用户名，请根据实际情况修改
```

**参数说明：**
- `-L 本地端口:远程主机:远程端口`：端口转发规则
- `-N`：不执行远程命令，仅转发端口
- `-f`：后台运行

#### 2.2 验证隧道连接

**在其他电脑上测试（隧道建立后）：**

```bash
# ============================================
# SSH隧道建立后，在其他电脑上使用 localhost 访问
# 不再使用 10.112.15.13，因为隧道已经映射了！
# ============================================

# 测试 MySQL（通过隧道，使用 localhost）
mysql -h localhost -P 3306 -urag_user -p
# 输入密码后应能正常连接

# 测试 Redis（通过隧道，使用 localhost）
redis-cli -h localhost -p 6379 -a redis_secure_password ping
# 预期输出: PONG

# 测试 Elasticsearch（通过隧道，使用 localhost）
curl http://localhost:9200/_cluster/health?pretty
# 预期输出: 集群健康信息

# 测试 Milvus（通过隧道，使用 localhost）
curl http://localhost:19530/healthz
# 预期输出: 健康状态

# 在浏览器打开 Milvus Web UI（通过隧道）
# 访问：http://localhost:9091
```

#### 2.3 在其他电脑上的 Python 代码连接（隧道模式）

```python
# ============================================
# SSH隧道建立后，在其他电脑上运行这段代码
# 注意：全部使用 localhost，不是 10.112.15.13
# ============================================

import pymysql
import redis
from elasticsearch import Elasticsearch
from pymilvus import connections

# MySQL 连接（通过 SSH 隧道）
conn = pymysql.connect(
    host='localhost',  # 使用 localhost！
    port=3306,
    user='rag_user',
    password='rag_password',
    database='rag_data'
)

# Redis 连接（通过 SSH 隧道）
r = redis.Redis(
    host='localhost',  # 使用 localhost！
    port=6379,
    password='redis_secure_password'
)

# Elasticsearch 连接（通过 SSH 隧道）
es = Elasticsearch(['http://localhost:9200'])  # 使用 localhost！

# Milvus 连接（通过 SSH 隧道）
connections.connect(
    alias="default",
    host='localhost',  # 使用 localhost！
    port=19530
)

print("所有服务连接成功！")
```

#### 2.4 管理 SSH 隧道

**查看隧道进程（在其他电脑上）：**

```bash
ps aux | grep "ssh -L"
```

**关闭隧道（在其他电脑上）：**

```bash
# 查找进程 ID
ps aux | grep "ssh -L" | grep -v grep

# 关闭指定进程
kill <PID>

# 关闭所有 SSH 隧道
pkill -f "ssh -L"
```

**自动重连（使用 autossh）：**

```bash
# 在其他电脑上安装 autossh
sudo apt install autossh  # Ubuntu/Debian
sudo yum install autossh  # CentOS/RHEL

# 使用 autossh（自动重连）
autossh -M 0 -f -N \
    -L 3306:localhost:3306 \
    -L 6379:localhost:6379 \
    -L 9200:localhost:9200 \
    -L 19530:localhost:19530 \
    yl@10.112.15.13
```

### 方法 3：使用 GUI 工具

#### MySQL 管理工具

**推荐工具：**
- **MySQL Workbench**（官方工具，免费）
- **DataGrip**（JetBrains 出品，功能强大）
- **DBeaver**（开源，跨平台）

**连接配置（以 MySQL Workbench 为例）：**

**方式1：直接连接**
```
Connection Name: RAG Server MySQL
Connection Method: Standard (TCP/IP)
Hostname: 10.112.15.13
Port: 3306
Username: rag_user
Password: rag_password
Default Schema: rag_data
```

**方式2：通过 SSH 隧道（推荐）**
```
Connection Name: RAG Server MySQL (SSH)
Connection Method: Standard TCP/IP over SSH

SSH 配置：
  SSH Hostname: 10.112.15.13:22
  SSH Username: yl
  SSH Password: 你的SSH密码

MySQL 配置：
  MySQL Hostname: 127.0.0.1（重要！）
  MySQL Server Port: 3306
  Username: rag_user
  Password: rag_password
```

**通过 SSH 隧道连接（DataGrip）：**

1. 新建连接 → MySQL
2. SSH Tunnel 标签页 → 启用 "Use SSH tunnel"
3. 填写 SSH 信息：
   - Host: 10.112.15.13
   - Port: 22
   - User: yl
   - Auth Type: Password
4. General 标签页：
   - Host: localhost（重要！）
   - Port: 3306
   - User: rag_user
   - Password: rag_password

#### Redis 管理工具

**推荐工具：**
- **RedisInsight**（官方工具，功能全面）
- **Another Redis Desktop Manager**（开源，轻量级）

**连接配置（RedisInsight）：**

**方式1：直接连接**
```
Host: 10.112.15.13
Port: 6379
Database Alias: RAG Server Redis
Password: redis_secure_password
```

**方式2：通过 SSH 隧道**
```
先在其他电脑建立SSH隧道：
ssh -L 6379:localhost:6379 yl@10.112.15.13 -N -f

然后连接配置：
Host: localhost
Port: 6379
Database Alias: RAG Server Redis (SSH)
Password: redis_secure_password
```

#### Elasticsearch 管理工具

**推荐方式：**

1. **Kibana**（官方工具，但需要单独部署）
2. **Elasticvue**（浏览器插件，轻量便捷）
3. **直接 API 访问**（使用 curl 或 Postman）

**Elasticvue Chrome 插件安装：**

```bash
# 1. 访问 Chrome 应用商店搜索 "Elasticvue"
# 2. 安装插件后，添加连接：
#    - Name: RAG Project ES
#    - Uri: http://服务器IP:9200（或 http://localhost:9200 + SSH 隧道）
```

**常用 API 操作：**

```bash
# 查看集群健康
curl http://localhost:9200/_cluster/health?pretty

# 查看所有索引
curl http://localhost:9200/_cat/indices?v

# 创建索引
curl -X PUT "http://localhost:9200/documents" -H 'Content-Type: application/json' -d'
{
  "settings": {
    "number_of_shards": 1,
    "number_of_replicas": 0
  }
}
'

# 搜索文档
curl -X GET "http://localhost:9200/documents/_search?pretty" -H 'Content-Type: application/json' -d'
{
  "query": {
    "match_all": {}
  }
}
'
```

#### Milvus 管理工具

**Attu（官方 Web UI）：**

Milvus 已内置 Web UI，可直接访问：

```
浏览器打开: http://服务器IP:9091
（或通过 SSH 隧道: http://localhost:9091）
```

**使用 Docker 运行独立的 Attu：**

```bash
docker run -d -p 8000:3000 \
  -e MILVUS_URL=服务器IP:19530 \
  zilliz/attu:latest

# 访问 http://localhost:8000
```

---

## 未来功能集成指南

### Redis 集成示例

#### 1. 缓存层（装饰器模式）

**安装依赖：**

```bash
pip install redis
```

**实现缓存装饰器：**

```python
# app/utils/cache.py
import redis
import json
import functools
from typing import Callable, Any
from app.core.config import settings

# 连接 Redis
redis_client = redis.Redis(
    host=settings.REDIS_HOST,
    port=settings.REDIS_PORT,
    password=settings.REDIS_PASSWORD,
    decode_responses=True
)

def cache_result(expire_time: int = 3600):
    """
    缓存函数结果装饰器

    Args:
        expire_time: 缓存过期时间（秒），默认 1 小时
    """
    def decorator(func: Callable) -> Callable:
        @functools.wraps(func)
        def wrapper(*args, **kwargs) -> Any:
            # 生成缓存键
            cache_key = f"{func.__name__}:{str(args)}:{str(kwargs)}"

            # 尝试从缓存获取
            cached = redis_client.get(cache_key)
            if cached:
                return json.loads(cached)

            # 执行函数并缓存结果
            result = func(*args, **kwargs)
            redis_client.setex(
                cache_key,
                expire_time,
                json.dumps(result, ensure_ascii=False)
            )
            return result

        return wrapper
    return decorator

# 使用示例
@cache_result(expire_time=1800)  # 缓存 30 分钟
def get_document_by_id(doc_id: int):
    # 从数据库查询（慢操作）
    return db.query(Document).filter(Document.id == doc_id).first()
```

#### 2. 会话管理

```python
# app/utils/session.py
import uuid
from datetime import timedelta

class RedisSession:
    def __init__(self, redis_client):
        self.redis = redis_client
        self.prefix = "session:"

    def create_session(self, user_id: int, expire_hours: int = 24) -> str:
        """创建用户会话"""
        session_id = str(uuid.uuid4())
        session_key = f"{self.prefix}{session_id}"

        self.redis.hset(session_key, mapping={
            "user_id": user_id,
            "created_at": datetime.now().isoformat()
        })
        self.redis.expire(session_key, timedelta(hours=expire_hours))

        return session_id

    def get_session(self, session_id: str) -> dict:
        """获取会话信息"""
        session_key = f"{self.prefix}{session_id}"
        return self.redis.hgetall(session_key)

    def delete_session(self, session_id: str):
        """删除会话（登出）"""
        session_key = f"{self.prefix}{session_id}"
        self.redis.delete(session_key)
```

#### 3. 任务队列（使用 Celery）

```bash
# 安装 Celery
pip install celery[redis]
```

```python
# app/tasks/celery_app.py
from celery import Celery
from app.core.config import settings

celery_app = Celery(
    'rag_tasks',
    broker=f'redis://:{settings.REDIS_PASSWORD}@{settings.REDIS_HOST}:{settings.REDIS_PORT}/0',
    backend=f'redis://:{settings.REDIS_PASSWORD}@{settings.REDIS_HOST}:{settings.REDIS_PORT}/1'
)

# 定义异步任务
@celery_app.task
def process_document_async(file_path: str):
    """异步处理文档上传和向量化"""
    # 1. 读取文档
    # 2. 向量化
    # 3. 存储到 Milvus
    # 4. 更新 MySQL 状态
    return {"status": "completed", "file": file_path}

# 使用示例
# from app.tasks.celery_app import process_document_async
# task = process_document_async.delay("/path/to/document.pdf")
# result = task.get(timeout=60)
```

### Elasticsearch 集成示例

#### 1. 全文搜索 API

**安装依赖：**

```bash
pip install elasticsearch llama-index-vector-stores-elasticsearch
```

**初始化 Elasticsearch：**

```python
# app/core/elasticsearch_client.py
from elasticsearch import Elasticsearch
from app.core.config import settings

es_client = Elasticsearch(
    [f'http://{settings.ES_HOST}:{settings.ES_PORT}']
)

# 创建文档索引
def create_document_index():
    index_body = {
        "settings": {
            "number_of_shards": 1,
            "number_of_replicas": 0,
            "analysis": {
                "analyzer": {
                    "ik_analyzer": {
                        "type": "custom",
                        "tokenizer": "ik_max_word"
                    }
                }
            }
        },
        "mappings": {
            "properties": {
                "title": {
                    "type": "text",
                    "analyzer": "ik_analyzer"
                },
                "content": {
                    "type": "text",
                    "analyzer": "ik_analyzer"
                },
                "file_path": {
                    "type": "keyword"
                },
                "upload_time": {
                    "type": "date"
                }
            }
        }
    }

    if not es_client.indices.exists(index="documents"):
        es_client.indices.create(index="documents", body=index_body)
```

#### 2. 混合检索（向量 + 全文）

```python
# app/services/hybrid_search.py
from typing import List, Dict
from llama_index.core import VectorStoreIndex
from elasticsearch import Elasticsearch

class HybridSearchService:
    def __init__(self, vector_index: VectorStoreIndex, es_client: Elasticsearch):
        self.vector_index = vector_index
        self.es = es_client

    def search(self, query: str, top_k: int = 10) -> List[Dict]:
        """
        混合检索：结合向量相似度和全文匹配

        Args:
            query: 查询文本
            top_k: 返回结果数量

        Returns:
            排序后的混合结果
        """
        # 1. 向量检索（语义相似度）
        vector_results = self.vector_index.as_query_engine().query(query)

        # 2. 全文检索（关键词匹配）
        es_results = self.es.search(
            index="documents",
            body={
                "query": {
                    "multi_match": {
                        "query": query,
                        "fields": ["title^2", "content"],
                        "type": "best_fields"
                    }
                },
                "size": top_k
            }
        )

        # 3. 结果融合（RRF - Reciprocal Rank Fusion）
        combined_results = self._merge_results(
            vector_results,
            es_results['hits']['hits'],
            top_k
        )

        return combined_results

    def _merge_results(self, vector_res, es_res, top_k):
        """融合两种检索结果"""
        # 实现 RRF 算法或简单的权重融合
        # ...
        pass
```

#### 3. 与 LlamaIndex 集成

```python
# app/rag/hybrid_retriever.py
from llama_index.core import VectorStoreIndex
from llama_index.vector_stores.elasticsearch import ElasticsearchStore

# 使用 Elasticsearch 作为向量存储（可选）
es_vector_store = ElasticsearchStore(
    es_url=f"http://{settings.ES_HOST}:{settings.ES_PORT}",
    index_name="llama_index_vectors"
)

vector_index = VectorStoreIndex.from_vector_store(es_vector_store)
query_engine = vector_index.as_query_engine()

# 查询
response = query_engine.query("如何部署 Docker 服务？")
print(response)
```

### 依赖包安装

**将以下依赖添加到 `requirements.txt`：**

```txt
# Redis 相关
redis>=5.0.0
celery[redis]>=5.3.0

# Elasticsearch 相关
elasticsearch>=8.11.0
llama-index-vector-stores-elasticsearch
```

**安装：**

```bash
pip install -r requirements.txt
```

---

## 数据持久化和备份

### Docker Volume 管理

**查看所有卷：**

```bash
docker volume ls | grep rag
```

**查看卷详情：**

```bash
docker volume inspect rag_project_mysql_data
docker volume inspect rag_project_redis_data
docker volume inspect rag_project_es_data
```

**卷存储位置（默认）：**

```
/var/lib/docker/volumes/rag_project_mysql_data/_data
/var/lib/docker/volumes/rag_project_redis_data/_data
/var/lib/docker/volumes/rag_project_es_data/_data
/var/lib/docker/volumes/rag_project_milvus_data/_data
```

### 备份策略

#### MySQL 备份

**手动备份：**

```bash
# 备份单个数据库
docker exec rag_mysql mysqldump -uroot -p"$(grep DB_ROOT_PASSWORD .env | cut -d'=' -f2)" rag_data > backup_$(date +%Y%m%d_%H%M%S).sql

# 备份所有数据库
docker exec rag_mysql mysqldump -uroot -p"$(grep DB_ROOT_PASSWORD .env | cut -d'=' -f2)" --all-databases > backup_all_$(date +%Y%m%d).sql
```

**恢复备份：**

```bash
# 恢复数据库
docker exec -i rag_mysql mysql -uroot -p"$(grep DB_ROOT_PASSWORD .env | cut -d'=' -f2)" rag_data < backup_20231218.sql
```

**自动备份脚本：**

```bash
#!/bin/bash
# backup_mysql.sh

BACKUP_DIR="/path/to/backups/mysql"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
DB_PASSWORD=$(grep DB_ROOT_PASSWORD .env | cut -d'=' -f2)

# 创建备份目录
mkdir -p $BACKUP_DIR

# 备份数据库
docker exec rag_mysql mysqldump -uroot -p"$DB_PASSWORD" rag_data > "$BACKUP_DIR/rag_data_$TIMESTAMP.sql"

# 压缩备份文件
gzip "$BACKUP_DIR/rag_data_$TIMESTAMP.sql"

# 删除 7 天前的备份
find $BACKUP_DIR -name "*.sql.gz" -mtime +7 -delete

echo "Backup completed: rag_data_$TIMESTAMP.sql.gz"
```

**设置定时任务（每天凌晨 2 点）：**

```bash
# 编辑 crontab
crontab -e

# 添加定时任务
0 2 * * * /path/to/backup_mysql.sh >> /var/log/mysql_backup.log 2>&1
```

#### Redis 备份

**Redis 自动生成备份文件（AOF 模式）：**

```bash
# 备份 Redis 数据文件
docker exec rag_redis redis-cli -a "$(grep REDIS_PASSWORD .env | cut -d'=' -f2)" SAVE

# 复制备份文件
docker cp rag_redis:/data/appendonly.aof ./redis_backup_$(date +%Y%m%d).aof
```

#### Elasticsearch 备份（快照）

```bash
# 创建快照仓库
curl -X PUT "http://localhost:9200/_snapshot/my_backup" -H 'Content-Type: application/json' -d'
{
  "type": "fs",
  "settings": {
    "location": "/usr/share/elasticsearch/backup"
  }
}
'

# 创建快照
curl -X PUT "http://localhost:9200/_snapshot/my_backup/snapshot_$(date +%Y%m%d)"
```

---

## 常见问题

### 1. 端口冲突

**现象：**
```
Error: bind: address already in use
```

**解决方案：**

```bash
# 查看端口占用情况
sudo lsof -i :3306
sudo lsof -i :6379
sudo lsof -i :9200

# 修改 .env 文件，更改端口
DB_PORT=13306
REDIS_PORT=16379
ES_PORT=19200

# 重新启动服务
docker-compose down
docker-compose up -d
```

### 2. Elasticsearch 启动失败

**现象：**
```
max virtual memory areas vm.max_map_count [65530] is too low, increase to at least [262144]
```

**解决方案：**

```bash
# 临时修改（重启后失效）
sudo sysctl -w vm.max_map_count=262144

# 永久修改
echo "vm.max_map_count=262144" | sudo tee -a /etc/sysctl.conf
sudo sysctl -p
```

### 3. Elasticsearch 内存不足

**现象：**
容器频繁重启，日志显示 OutOfMemoryError

**解决方案：**

修改 `docker-compose.yml` 中的内存配置：

```yaml
elasticsearch:
  environment:
    - "ES_JAVA_OPTS=-Xms256m -Xmx256m"  # 降低到 256MB
```

### 4. 容器无法访问网络

**现象：**
```
Could not resolve host: docker.elastic.co
```

**解决方案：**

```bash
# 检查 DNS 配置
docker run --rm busybox nslookup google.com

# 修改 Docker 守护进程配置
sudo vim /etc/docker/daemon.json

# 添加 DNS 服务器
{
  "dns": ["8.8.8.8", "8.8.4.4"]
}

# 重启 Docker
sudo systemctl restart docker
```

### 5. 防火墙配置（如需外网访问）

**Ubuntu/Debian（ufw）：**

```bash
sudo ufw allow 3306/tcp
sudo ufw allow 6379/tcp
sudo ufw allow 9200/tcp
sudo ufw allow 19530/tcp
sudo ufw reload
```

**CentOS/RHEL（firewalld）：**

```bash
sudo firewall-cmd --permanent --add-port=3306/tcp
sudo firewall-cmd --permanent --add-port=6379/tcp
sudo firewall-cmd --permanent --add-port=9200/tcp
sudo firewall-cmd --permanent --add-port=19530/tcp
sudo firewall-cmd --reload
```

### 6. Redis 密码认证失败

**现象：**
```
NOAUTH Authentication required
```

**解决方案：**

```bash
# 检查 .env 中的密码是否正确
grep REDIS_PASSWORD .env

# 测试连接
docker exec rag_redis redis-cli -a "your_password" ping
```

### 7. MySQL 字符集问题（中文乱码）

**解决方案：**

```bash
# 检查字符集配置
docker exec rag_mysql mysql -uroot -p -e "SHOW VARIABLES LIKE 'character%'"

# 应显示 utf8mb4，如果不是，重新创建数据库
docker exec rag_mysql mysql -uroot -p -e "
ALTER DATABASE rag_data CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci;
"
```

---

## 服务管理命令

### 基本操作

```bash
# 启动所有服务
docker-compose up -d

# 启动指定服务
docker-compose up -d redis elasticsearch

# 停止所有服务
docker-compose stop

# 停止指定服务
docker-compose stop redis

# 重启服务
docker-compose restart redis

# 查看服务状态
docker-compose ps

# 查看服务日志
docker-compose logs -f redis

# 查看所有服务日志
docker-compose logs -f

# 进入容器
docker exec -it rag_redis sh
docker exec -it rag_mysql bash
```

### 清理操作

```bash
# 停止并删除容器（保留数据卷）
docker-compose down

# 停止并删除容器和数据卷（危险操作！）
docker-compose down -v

# 删除未使用的镜像
docker image prune -a

# 删除未使用的卷
docker volume prune

# 完全清理 Docker 系统
docker system prune -a --volumes
```

### 资源监控

```bash
# 查看容器资源使用情况
docker stats

# 查看特定容器
docker stats rag_redis rag_elasticsearch

# 查看磁盘使用
df -h | grep docker

# 查看卷使用情况
docker system df -v
```

### 日志管理

```bash
# 实时查看日志
docker-compose logs -f --tail=100 redis

# 导出日志到文件
docker-compose logs redis > redis.log

# 清理日志（需要重启容器）
docker-compose down
docker-compose up -d
```

---

## 性能调优建议

### 开发环境

- Elasticsearch 内存：512MB（当前配置）
- Redis 无需额外配置
- MySQL 连接池：20-50 连接

### 生产环境

1. **启用 Elasticsearch 安全认证**
   ```yaml
   environment:
     - xpack.security.enabled=true
     - ELASTIC_PASSWORD=your_strong_password
   ```

2. **使用强密码**（MySQL、Redis）

3. **配置资源限制**
   ```yaml
   services:
     elasticsearch:
       deploy:
         resources:
           limits:
             cpus: '2.0'
             memory: 2G
           reservations:
             memory: 1G
   ```

4. **定期备份数据**

5. **监控磁盘空间**
   ```bash
   df -h | grep docker
   ```

6. **使用日志轮转**
   ```json
   // /etc/docker/daemon.json
   {
     "log-driver": "json-file",
     "log-opts": {
       "max-size": "10m",
       "max-file": "3"
     }
   }
   ```

---

## 故障排查

### 服务无法启动

1. 查看日志：`docker-compose logs <service_name>`
2. 检查端口占用：`sudo lsof -i :<port>`
3. 验证配置：`docker-compose config`
4. 检查磁盘空间：`df -h`

### 连接超时

1. 检查防火墙：`sudo ufw status`
2. 验证网络：`docker network ls`
3. 测试连接：`telnet 服务器IP 端口`

### 性能问题

1. 查看资源使用：`docker stats`
2. 检查慢查询日志（MySQL）
3. 分析 Elasticsearch 索引状态
4. 优化 Milvus 索引参数

---

## 联系方式

如有问题或建议，请：

- 提交 Issue：[项目 GitHub Issues]
- 邮件联系：[your-email@example.com]
- 查看文档：[项目文档链接]

---

## 版本历史

- **v1.0.0** (2023-12-18)
  - ✅ 初始版本发布
  - ✅ MySQL 8.0 + Milvus 2.3.13 部署
  - ✅ Redis 7.2 + Elasticsearch 8.11 支持
  - ✅ 完整的 SSH 隧道访问指南
  - ✅ 未来功能集成示例

---

**最后更新时间：** 2023年12月18日
